...
object kmeansQA {
  def main(args: Array[String]): Unit = {
    ...
    val q = scala.io.StdIn.readLine().toLowerCase
    if (q.contains("kmeans") && q.contains("error")) {
      System.out.println("The kmeans error is::" + error)

    }

    if (q.contains("kmeans") && q.contains("clusters") && q.contains("iterations")) {
      System.out.println("The number of clusters are::" + k)
      System.out.println("The number of clusters are::" + numIterations)

    }
    if (q.contains("kmeans") && q.contains("output")) {

      System.out.println("The kmeans output is:")
      resultsA.foreach(f => {
        System.out.println(f._1._2 + ";" + f._2)
