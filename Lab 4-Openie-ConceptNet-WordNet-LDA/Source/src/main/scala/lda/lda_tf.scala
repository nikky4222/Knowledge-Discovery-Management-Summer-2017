  ...
  /**
    ...
    */
    ...
    private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) = {
      ...
      val df = sc.textFile(paths.mkString(",")).map(f => f.split(" ")).flatMap(f=>f)
      ...
      */
      ...
      val vocab=df.distinct().collect()
      (tf1, vocab, dff.count()) // Vector, Vocab, total token count
    }
  }
